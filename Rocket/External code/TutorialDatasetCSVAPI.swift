...
/// Conform `Anton.GameParameters` to `TensorGroup` so that we can load it into a `Dataset`.
extension Anton.GameParameters: TensorGroup {
...
extension Dataset where Element == Anton.GameParameters {
  init(
    ...
    self.init(elements: Anton.GameParameters(features: featuresTensor, labels: labelsTensor))
  }
}

/// Sequence doesn't have a non-predicated `first` property, so we define one.
/// TODO: Add this to Swift's stdlib.
extension Sequence where Element == Anton.GameParameters {
  var first: Anton.GameParameters? {
